...
from mcp.shared import convert, grpc_utils
...
class McpServicer(mcp_pb2_grpc.McpServicer):
    ...
    async def ListResourceTemplates(
        ...
        try:
            ...
            resource_template_protos = convert.resource_template_types_to_protos(resource_templates)
    ...
    async def ReadResource(
        ...
        try:
            ...
            resource_contents_proto_list = convert.read_resource_content_to_proto(request.uri, content_items)
    ...
    async def tool_runner(
        ...
        try:
            ...
            results = await self.mcp_server.call_tool(
                tool_name,
                arguments,
                request_context=req_context,  # type: ignore
            )

            try:
                unstructured_content, maybe_structured_content = convert.normalize_and_validate_tool_results(
                    results, tool
                )
            except convert.ToolOutputValidationError as e:
                await self._make_error_result(response_queue, str(e))
                return

            call_tool_response = mcp_pb2.CallToolResponse(common=mcp_pb2.ResponseFields())
            if unstructured_content:
                call_tool_response.content.extend(
                    convert.unstructured_tool_output_to_proto(list(unstructured_content))  # type: ignore
                )
            if maybe_structured_content:
                json_format.ParseDict(maybe_structured_content, call_tool_response.structured_content)
    ...
    async def _make_error_result(
        ...
        content = mcp_pb2.CallToolResponse.Content(text=mcp_pb2.TextContent(text=error_message))
