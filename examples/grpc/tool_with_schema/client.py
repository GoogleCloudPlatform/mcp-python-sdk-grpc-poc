...
async def call_server(host: str, port: int):
    ...
    try:
        ...
        print("\n========================================")
        print("➡️ Calling weather_tool with arguments: location='London', unit='celsius'...")
        print("========================================")
        try:
            weather_response = await session.call_tool("weather_tool", {"location": "London", "unit": "celsius"})
        ...
        print("========================================")
        try:
            calc_response = await session.call_tool("calculator_tool", {"a": 10.0, "b": 5.0})
            print(f"✅ Received calculator_tool response:\n   Result: {calc_response}")
        except mcp.McpError as e:
        ...
        try:
            ...
            if result.isError:
                print(f"✅ Received expected failure for weather_tool invalid input:\n   {result}")
            else:
        ...
        print("\n========================================")
        print("➡️ Calling bad_calculator_tool with output schema mismatch: a=1.0, b=2.0...")
        print("========================================")
        try:
            result = await session.call_tool("bad_calculator_tool", {"a": 1.0, "b": 2.0})
            if result.isError:
                print(f"✅ Received expected failure for bad_calculator_tool invalid output:\n   {result}")
            else:
